...
#         return (
#             "Hello! I am GemAI, here to help you explore Prashant's portfolio.\n\n"
#             "Try asking:\n• What are Prashant's projects related to AI?\n• Summarize his resume\n• How to contact him?"
#         )
#     if "thank" in q_clean:
#         return "You're welcome! 😊 Let me know if you'd like help with his resume, skills, or contact info."
#     if "how are you" in q_clean or "how you doing" in q_clean:
#         return "I'm doing great! Let me know how I can assist with Prashant's profile."
#     if any(p in q_clean for p in ["who are you", "are you ai", "your name", "are you human"]):
#         return "I am GemAI — the AI agent embedded in Prashant Baral's portfolio. Ask me anything about his skills, resume, or work."
#     if "sorry" in q_clean:
#         return "No worries at all! Let me know if you need help exploring Prashant's profile."
#     if "personal life" in q_clean:
#         return "Sorry, I couldn't find personal life details in Prashant's profile. But I can share his hobbies if you'd like!"
#     return None
...
# portfolio_docs = load_portfolio_docs()
# docx_text = extract_text_from_docx("./public/PrashantResume.docx")
# resume_doc = Document(page_content=docx_text, metadata={"source": "Resume DOCX"})
# splitter = CharacterTextSplitter(chunk_size=1000, chunk_overlap=50)
# split_docs = splitter.split_documents(portfolio_docs + [resume_doc])
# embeddings = HuggingFaceEmbeddings(model_name="all-MiniLM-L6-v2")
...
#     if (resp := handle_small_talk(query)):
...
#     matched = match_keywords(qnorm, keywords, aliases)
#     if any(re.search(p, qnorm) for p in [
#         r"(detail|details|about|info)\s+(of|on)?\s*(Prashant|his)",
#         r"(describe|tell)\s+(about\s+)?(Prashant|his)",
#         r"who is he", r"who is prashant"
#     ]):
#         matched += ["qualification.docx", "interests.docx"]

#     extra_context = [doc for doc in portfolio_docs if doc.metadata["source"] in matched]
#     if not any(doc.metadata["source"] == "skills.docx" for doc in extra_context):
#         for doc in portfolio_docs:
#             if doc.metadata["source"] == "skills.docx":
#                 extra_context.append(doc)
...
#     context = "\n\n".join([doc.page_content for doc in combined_docs])
#     prompt = f"""
# You are **GemAI**, the embedded AI agent for Prashant Baral's portfolio. Only use the context below. NEVER guess.

# 📌 Do NOT say "I am Prashant." Refer to him as "Prashant Baral".
# 📌 If info not found, respond with: "I could not find information related to that."
# 📌 Use these links:
# - [Resume (PDF)](/files/PrashantResume.pdf)
# - [Personal Email](mailto:thedashh7@gmail.com)
# - [University Email](mailto:pb23395n@pace.edu)
...
# 📌 Respond naturally and clearly. Use markdown only. No hallucinations.
...
# - **Write a SINGLE short paragraph, no more than 3 to 4 concise lines.**
# - combine details from :
# - qualification.docx + resume for education/work
# - interests.docx for personal interests
# - Keep it brief, clear, and professional
# - End with GitHub, LinkedIn, and resume links
...
#     session = SessionLocal()
#     log = GemAILog(session_id=session_id, timestamp=datetime.utcnow().isoformat(), question=query, response=answer)
#     session.add(log)
#     session.commit()
#     session.close()
